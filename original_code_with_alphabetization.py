...
dirWithXML = 'NLCTrainNoDupes'  # Also to be basis for csv file name
# baseDir = 'C:/Users/Tara/PycharmProjects/untitled/moreToTransform/' + dirWithXML + '/'  # directory with transformed xml file directories
baseDir = 'C:/Users/Tara/PycharmProjects/untitled/' + dirWithXML + '/'  # directory with transformed xml file directories
...
def get_values_from_xml_file(xml_file, the_list):
    """
    ...
    try:
        ...
        for patent in transform_tree.iter('patent-info'):
            ...
            for ndx, member in enumerate(the_list):
                try:
                    value = patent.find(the_list[ndx]).text.encode('utf8')
                    the_list[ndx] = value
                except Exception, e:
...
def add_to_storage(xml_file, storage, search_term):
    try:
        ...
        if ucid.startswith('D'):
            ...
            cleaned_stemmed = text_cleanup.normalize_and_stem(available_text)
...
def SummarizeAbstractToMaxLength(abstract, maxLength):
    ...
    for sentence in abstractSummary:
        if (len(abstract) + len(
                unicode(sentence))) < maxLength:  # Trick to only add enough sentences to stay under 1024 chars
